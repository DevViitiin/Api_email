...
def build_action_email_reported(data: dict) -> tuple:
    ...
    if action == "ban":
        ...
        detail = (
            ...
            f"foi <strong style='color:#ef4444;'>banida permanentemente</strong> da plataforma. "
...
def build_archive_email_reported(data: dict) -> tuple:
    name = data.get("reported_name", "Usuário")
    aid  = data.get("archive_id", "N/A")
    date_str = data.get("start_br") or datetime.now().strftime("%d/%m/%Y")
    ...
    body = f"""
      ...
      <div style="background:#1a1c28;border:1px solid #1f2230;border-radius:10px;
        ...
        <p style="font-size:13px;color:#8b8fa8;line-height:1.6;margin:0;">
          ...
          evidências disponíveis, <strong style="color:#ffffff;">não foram encontradas
          irregularidades suficientes</strong> para justificar medidas punitivas.
    ...
    return (
        f"[MãoDeObra] Análise de denúncia encerrada — Protocolo {aid}",
        _base_template("Análise Concluída", body),
    )


# ── Template: qualquer ação → denunciante ─────────────────────────────────────
def build_reporter_email(data: dict) -> tuple:
    ...
    return (
        f"[MãoDeObra] Sua denúncia foi processada — Protocolo {aid}",
